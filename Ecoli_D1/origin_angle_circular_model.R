...
data_table <- read_tsv('lb_origin_angle_scaled.txt')
...
write.table(data.frame(gene=data_table$...1, angle_pred=angle_pred), 'lb_predicted_angle.txt', sep="\t", quote=F, row.names=F)

# Save model
saveRDS(fit, 'lb_stanfit.rds')
...
angle_mean(A_pred) # Gives -2.511148
...
angle_mean(A_pred) # Gives -3.071179

# Polymerase speed
chrom_length <- 4641652
td = 26 # Doubling time in min

((chrom_length / 2) / mean(k)) / (td * 60) # 777.9088 bp/s
